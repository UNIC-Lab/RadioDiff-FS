...
class Unet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.phase_threshold = cfg.get('phase_threshold', 500)
        self.cond_boost_factor = cfg.get('cond_boost_factor', 0.3)
    ...
    def forward(self, x, time, mask, x_self_cond = None, **kwargs):
        ...
        if hasattr(self, 'phase_threshold') and hasattr(self, 'cond_boost_factor'):
            high_t = (time > self.phase_threshold).float().view(-1, 1, 1, 1)
            device = hm[0].device
            high_t = high_t.to(device)
            cond_boost_factor = torch.tensor(self.cond_boost_factor, device=device, dtype=high_t.dtype)
            hm = [h * (1 + high_t * cond_boost_factor) for h in hm]
